...
# --- RAG Prompt Template (Modified to include chat history) ---
# IMPORTANT: Added {chat_history} to the prompt template.
# The RunnableWithMessageHistory will automatically format the history and pass it here.
diet_prompt = PromptTemplate.from_template("""
You are an AI assistant specialized in Indian diet and nutrition.
Based on the following conversation history and the user's query, provide a simple, practical, and culturally relevant food suggestion suitable for Indian users.
...
""")
logging.info("RAG Prompt template created with chat history support.")
...
try:
    ...
    qa_chain = RetrievalQA.from_chain_type(
        ...
        retriever=db.as_retriever(search_kwargs={"k": 3}), # Retriever fetches top 3 relevant documents.
        chain_type="stuff", # 'stuff' chain type passes all retrieved documents into the prompt.
        chain_type_kwargs={"prompt": diet_prompt}, # Use the custom prompt defined above.
...
Prioritize the Primary RAG Answer as the core information if it seems relevant and helpful. If the RAG answer is generic, error-related, or off-topic, rely more heavily on the Additional Suggestions.
Incorporate useful and relevant details from the Additional Suggestions if they enhance the practicality and Indian relevance of the advice.
...
def groq_diet_answer(model_name: str, query: str, user_diet_preference: str = "any") -> str:
    """
    Fetches a diet suggestion from a specified Groq model.
    Args:
        model_name: The short name of the Groq model (e.g., "llama", "mixtral").
        query: The user's query.
        user_diet_preference: User's dietary preference ("vegetarian", "non-vegetarian", "any").
    Returns:
    ...
    try:
        ...
        # Refined prompt for Groq models - make it more general purpose
        prompt_content = f"User query: '{query}'. Provide a concise, practical Indian diet suggestion or food item"
        if user_diet_preference == "vegetarian":
            prompt_content += " (vegetarian)"
        elif user_diet_preference == "non-vegetarian":
            prompt_content += " (non-vegetarian)"
        # Removed the specific mention of "piles" here to avoid irrelevant suggestions if the query isn't about it.
        prompt_content += ", considering regional relevance if possible. Be brief."
        ...
        logging.info(f"Calling Groq API with model: {actual_model_name} for query: '{query}' with preference: {user_diet_preference}")
...
# Use Streamlit's caching to avoid repeated Groq calls for the same query and preference
@st.cache_data(show_spinner=False, ttl=3600) # Cache for 1 hour
def cached_groq_answers(query: str, diet_preference: str) -> dict:
    """
    Fetches and caches suggestions from multiple Groq models concurrently.
    """
    logging.info(f"Fetching cached Groq answers for query: '{query}', preference: '{diet_preference}'")
    ...
    with ThreadPoolExecutor(max_workers=len(models)) as executor:
        future_to_model = {executor.submit(groq_diet_answer, name, query, diet_preference): name for name in models}
...
# --- Greeting and Preference Helpers ---
...
def extract_diet_preference(query: str) -> str:
    """Extracts diet preference (vegetarian, non-vegetarian) from the query."""
...
if 'session_id' not in st.session_state:
    ...
    get_session_history(st.session_state.session_id)
    logging.info(f"Streamlit session state has existing session_id: {st.session_state.session_id}. Langchain history loaded.")
    ...
    if is_greeting(query):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                try:
                    logging.info(f"Invoking conversational_qa_chain for session: {st.session_state.session_id}")
                ...
                user_preference = extract_diet_preference(query)
                logging.info(f"Extracted diet preference: {user_preference}")

                llama_suggestion = mixtral_suggestion = gemma_suggestion = ""
                if use_llms_toggle:
                    if GROQ_API_KEY:
                        # Use cached function for Groq calls
                        groq_results = cached_groq_answers(query, user_preference)
                ...
                try:
                    # Adjust the merge prompt based on whether RAG was successful
                    if not rag_success or "Could not retrieve a specific answer" in rag_answer:
                         logging.warning("RAG was not successful or returned a generic error. Relying more on Groq suggestions.")
                         # Modify prompt slightly to indicate RAG issue
                         merge_prompt_template_fallback = """
You are a diet planning assistant.
It seems there was an issue retrieving detailed information from the primary source.
Please synthesize the following AI suggestions into a single, coherent, and practical Indian diet plan or suggestion.
Focus on incorporating useful and relevant details from the suggestions to create actionable advice tailored for Indian users, using simple language and common food items.
Structure the response clearly.
...
Refined and Merged Diet Plan/Suggestion (Based on available suggestions):
"""
                         merge_prompt_fallback = PromptTemplate.from_template(merge_prompt_template_fallback)
                         final_prompt_text = merge_prompt_fallback.format(
                             llama=llama_suggestion,
                             mixtral=mixtral_suggestion,
                             gemma=gemma_suggestion
                         )
                    else:
                         logging.info("RAG was successful. Using the standard merge prompt.")
